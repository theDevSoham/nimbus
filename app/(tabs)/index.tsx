...
export default function WeatherAndNews() {
  ...
  const { settings } = useSettings(); // 🎯 pull unit + newsMode from context
  ...
  useEffect(() => {
    const loadWeatherAndNews = async () => {
      ...
      try {
        ...
        // ✅ use unit from settings
        const currentWeather = await getCurrentWeather(
          latitude,
          longitude,
          settings.unit // "celsius" or "fahrenheit"
        );
        ...
        // ✅ use newsMode from settings
        const filter = settings.newsMode === "weatherBased";
        const news = await fetchFilteredNews("us", type, filter);
        setArticles(news);
      } catch (err) {
    ...
    loadWeatherAndNews();
  }, [location, settings]); // 🎯 re-run when user changes settings
